#!/bin/bash
# start-all-services.sh - Comprehensive HelixAgent Services Startup Script
# Brings up ALL HelixAgent containers and services
...
echo "=============================================="
echo "HelixAgent - Complete Services Startup"
echo "=============================================="
echo ""
...
if [ "${START_BIGDATA:-false}" = "true" ]; then
...
if [ "${START_SECURITY:-false}" = "true" ]; then
...
$RUNTIME ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep helixagent || echo "No helixagent containers running"
...
echo "=========================================="
echo ""
echo "Core services are now running!"
echo ""
echo "Quick verification:"
echo "  • PostgreSQL:  podman exec -it helixagent-postgres pg_isready"
echo "  • Redis:       podman exec -it helixagent-redis redis-cli ping"
