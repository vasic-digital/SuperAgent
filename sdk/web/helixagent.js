/**
 * HelixAgent Protocol Enhancement SDK
 * JavaScript/TypeScript client library for HelixAgent Protocol Enhancement API
 *
 * @version 1.0.0
 * @author HelixAgent
 * @license MIT
...
class HelixAgentUtils {
    static async createUnifiedClient(baseURL, apiKey) {
        return new HelixAgentClient(baseURL, { apiKey });
    }

    static async initializeDefaultPlugins(client) {
        try {
            // Load default MCP plugin
            await client.loadPlugin('/opt/helixagent/plugins/mcp-basic-integration.so');

            // Load default LSP plugin
            await client.loadPlugin('/opt/helixagent/plugins/lsp-code-completion.so');

            // Load default ACP plugin
            await client.loadPlugin('/opt/helixagent/plugins/acp-agent-orchestration.so');
                ...
                const monitor = async () => {
                    ...
                    try {
                        ...
                        if (typeof window !== 'undefined' && window.dispatchEvent) {
                            window.dispatchEvent(new CustomEvent('helixagent:metrics', {
...
if (typeof module !== 'undefined' && module.exports) {
    // CommonJS
    module.exports = { HelixAgentClient, HelixAgentUtils };
} else if (typeof define === 'function' && define.amd) {
    // AMD
    define([], function() {
        return { HelixAgentClient, HelixAgentUtils };
    });
} else if (typeof window !== 'undefined') {
    // Browser global
    window.HelixAgent = { HelixAgentClient, HelixAgentUtils };
}