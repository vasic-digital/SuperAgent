...
init_challenge "$CHALLENGE_NAME" "Provider $(echo ${challenge#provider_} | sed 's/_/ /g' | sed 's/\b\(.\)/\u\1/g') Challenge"
load_env
log_info "Testing provider $(echo ${challenge#provider_} | sed 's/_/ /g')..."
test_functionality() {
    ...
    [[ "$(echo "$resp" | tail -n1)" == "200" ]] && record_assertion "test" "works" "true" "Working" || record_assertion "test" "works" "false" "Failed"
}
main() {
