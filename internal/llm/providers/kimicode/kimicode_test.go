...
func TestNewProvider(t *testing.T) {
	...
	assert.Equal(t, "default-model", provider.model)
}

func TestNewProviderWithCustomURL(t *testing.T) {
	customURL := "https://custom.api.com/v1/chat/completions"
	provider := NewKimicodeProvider("test-key", customURL, "custom-model")
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewKimicodeProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	provider := NewKimicodeProvider("test-api-key", server.URL, "")
	...
	assert.Equal(t, "kimicode", resp.ProviderID)
}
...
func TestCompleteWithError(t *testing.T) {
	...
	provider := NewKimicodeProvider("invalid-key", server.URL, "")
...
func TestCompleteStream(t *testing.T) {
	...
	provider := NewKimicodeProvider("test-key", server.URL, "")
...
func TestGetCapabilities(t *testing.T) {
	provider := NewKimicodeProvider("test-key", "", "")
...
func TestValidateConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider := NewKimicodeProvider(tt.apiKey, "", "")
