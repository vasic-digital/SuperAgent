...
func (f *CSVFormatter) FormatDebateTeamIntroduction(topic string, members []*services.DebateTeamMember) string {
	...
	// CSV header
	_ = w.Write([]string{"Position", "Role", "Model", "Provider", "Fallback_Model", "Fallback_Provider"}) //nolint:errcheck
	...
	for _, member := range members {
		...
		_ = w.Write([]string{ //nolint:errcheck
...
func (f *CSVFormatter) FormatPhaseHeader(phase services.ValidationPhase, phaseNum int) string {
	...
	_ = w.Write([]string{"Phase_Type", "Phase_Num", "Icon", "Timestamp"})                                                         //nolint:errcheck
	_ = w.Write([]string{string(phase), fmt.Sprintf("%d", phaseNum), getPhaseIcon(phase), time.Now().UTC().Format(time.RFC3339)}) //nolint:errcheck
...
func (f *CSVFormatter) FormatPhaseContent(content string) string {
	...
	_ = w.Write([]string{"Content", "Timestamp"})                         //nolint:errcheck
	_ = w.Write([]string{content, time.Now().UTC().Format(time.RFC3339)}) //nolint:errcheck
...
func (f *CSVFormatter) FormatFinalResponse(content string) string {
	...
	_ = w.Write([]string{"Type", "Content", "Timestamp"})                                   //nolint:errcheck
	_ = w.Write([]string{"final_response", content, time.Now().UTC().Format(time.RFC3339)}) //nolint:errcheck
