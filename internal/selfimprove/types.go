...
type RewardModel interface {
	...
	// Train updates the reward model with feedback
...
type AggregatedFeedback struct {
	...
	ScoreDistribution map[string]int              `json:"score_distribution"` // Buckets
...
type PolicyUpdate struct {
	ID               string                 `json:"id"`
	OldPolicy        string                 `json:"old_policy"` // Old system prompt/config
	NewPolicy        string                 `json:"new_policy"` // Updated policy
	UpdateType       PolicyUpdateType       `json:"update_type"`
	Reason           string                 `json:"reason"`
	ImprovementScore float64                `json:"improvement_score"`
	Examples         []*TrainingExample     `json:"examples"` // Examples that led to this
...
type DebateRewardEvaluator interface {
	// EvaluateWithDebate uses AI debate to evaluate response quality
	EvaluateWithDebate(ctx context.Context, prompt, response string) (*DebateEvaluation, error)

	// CompareWithDebate uses debate to compare two responses
